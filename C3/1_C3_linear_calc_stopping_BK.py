#0_C3_calc_stopping_BK.py: Brandt-Kitagawaモデルを3原子クラスターの阻止能公式に適用し、イオンに対する阻止能を計算する(積分はscipy.integrateを利用)
...
q_ave_path = os.path.join(input_dir, '0_C3_linear_average_charge.txt')
...
def set_parameters(path):
    ...
    E = params["E0"]
    v = sqrt(E/E_CARBON)
    r = {k:v/a_0 for k, v in params["r"]["linear"].items()} # to atomic unit
...
def main():
    ...
    #ファイルに書き込み
    output_filename = 'E={}keV_atom_C3_linear_{}.txt'.format(E, target)
